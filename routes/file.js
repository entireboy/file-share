...
exports.download = function(req, res) {
  ...
  mongo.fetchCollection(FILE_COLLECTION, function(err, collection) {
    collection.findOne({_id: new require.main.db.ObjectID(fileId)}, function(err, doc) {
      if(err || null === doc) {
...
/**
 * 사용자가 권한을 가지고 있는 모든 종류의 파일을 일부(5개 등 config에 설정된 개수)만 가져온다. (종류: own, viewable, ...)
 * @param {http.ServerRequest} req HTTP request
 * @param {http.ServerResponse} res HTTP response
...
exports.list.ofUser = function(req, res, result) {
  mongo.fetchCollection(FILE_COLLECTION, function(err, collection) {
    var totalFileTypeCount = 2; // owns, views
